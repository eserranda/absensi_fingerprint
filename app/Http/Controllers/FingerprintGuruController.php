...
class FingerprintGuruController extends Controller
{
        ...
        if ($request->ajax()) {
            ...
            return DataTables::of($data)
                ...
                ->addColumn('action', function ($row) {
                    $actionBtn = ' 
                    <a href="fingerprint_guru/detail/' . $row->id . '" class="btn btn-sm btn-info btn-icon" aria-label="Button">
    ...
    public function show(FingerprintGuru $fingerprintGuru, $id)
    {
        $data = $fingerprintGuru::find($id);
        return view('fingerprint_guru.detail', compact('data'));
    }
    ...
    public function status_deleted($modul, $id_finger)
    {
        ...
        if ($getStatusDeleted->deleted_status == 1) {
            ...
            $delete = FingerprintGuru::where('id_modul_fingerprint', $getStatusDeleted->id)
